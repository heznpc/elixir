    ...
    def do_one(rec, tier, model):
        ...
        while True:
            ...
            if resp.get("is_error"):
                ...
                # Non-quota is_error (or quota retries exhausted): write an error
                # record with the actual error context and return — do NOT fall
                # through to parse(text), which silently masquerades the error
                # as parse_error='no_json'.
                rec_out = {"openalex_id":rec["openalex_id"],"tier":tier,"model":model,
                           "verdict":"","domains":[],"reason":"",
